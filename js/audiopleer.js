...
audio.addEventListener('ended', playNext); //после окончания одного плейлиста идет следующее
...
function changeColor() { //сперва удаляю этот класс у всех, а потом добавляю текущему
...
function customPlayer() {
    ...
    //обновляется прогресс-бар и время проигрывания
    setInterval(() => {
        const progressBar = document.querySelector("input");
        progressBar.value = audio.currentTime / audio.duration * 100;
    
        const currentTime = document.querySelector('.current');
        currentTime.textContent = getTimeCodeFromNum(audio.currentTime); //функция перевода секунд в минуты
      }, 50);
}
...
function changeBtnVolume() {
    ...
    if (btnVolum.classList.contains("volume-button")) {
        ...
        if (btnVolum.classList.contains("volume-button-off")) {
            const volumePersentage = document.querySelector(".volume-percentage");
